...
def ComputeTorsionHistograms(customTorsionProfiles, binsize):
    """
    Compute histograms for custom torsion profiles. The torsion angles are expected to be in radians
    in the range ``[0, 2π]``.

    :param customTorsionProfiles: A 2D numpy array where each column represents a torsion angle profile.
    :param binsize: The size of the bins for the histogram.
    :return: A tuple containing:
        - **hists**: A list of histograms for each torsion angle profile.
        - **histsCount**: A list of counts for each histogram.
        - **xhist**: The bin edges for the histograms.
    """
...
def ComputeGaussianFit(xHist, yHist, yHistCount, binsize, sigma=np.pi*0.3, **kwargs):
    ...
    :param xHist: Bin edges for the histogram.
    :param yHist: Histogram density values.
    :param yHistCount: Histogram count values.
    :param binsize: Size of the bins used in the histogram.
    :param sigma: Standard deviation for the Gaussian smoothing (default is ``0.3π``).
    :param kwargs: Additional parameters for peak partitioning.
    :return: A tuple containing:
        - **coeffs**: Coefficients of the fitted Gaussian functions.
        - **bounds**: Boundaries of the fitted peaks.
    """
